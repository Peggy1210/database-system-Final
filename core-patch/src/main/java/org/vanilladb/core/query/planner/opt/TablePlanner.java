...
class TablePlanner {
	...
	public Plan makeSelectPlan() {
		...
		// We don't use sort plan
		// if (embField != null) {
		// 	// Map<String, ConstantRange> range = new HashMap<String, ConstantRange>();
		// 	// range.put(embField.fieldName(), ConstantRange.newInstance(embField.query(), true, embField.query(), true));
		// 	// p = new IndexSelectPlan(p, embField.fieldName(), range, tx);
	...
	private Plan makeIndexSelectPlan() {
		return IndexSelector.selectByBestMatchedIndex(tblName, tp, pred, tx, embField);
	}
