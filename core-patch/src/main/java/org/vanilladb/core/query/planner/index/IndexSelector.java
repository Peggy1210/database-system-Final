...
public class IndexSelector {
	...
	public static IndexSelectPlan selectByBestMatchedIndex(String tblName,
			TablePlan tablePlan, Predicate pred, Transaction tx, DistanceFn embFields) {
		
		Set<IndexInfo> candidates = new HashSet<IndexInfo>();
		for (String fieldName : VanillaDb.catalogMgr().getIndexedFields(tblName, tx)) {
			ConstantRange searchRange = pred.constantRange(fieldName);
			if (searchRange == null && embFields == null)
				continue;
		...
		return selectByBestMatchedIndex(candidates, tablePlan, pred, tx, embFields);
	}
	...
	public static IndexSelectPlan selectByBestMatchedIndex(Set<IndexInfo> candidates,
			TablePlan tablePlan, Predicate pred, Transaction tx, DistanceFn embField) {
		// Choose the index with the most matched fields in the predicate
		int matchedCount = 0;
		IndexInfo bestIndex = null;
		Map<String, ConstantRange> searchRanges = null;
		...
		for (IndexInfo ii : candidates) {
			...
			for (String fieldName : ii.fieldNames()) {
				...
				// support embedding
				if (embField != null && fieldName.equals(embField.fieldName())) {
					Map<String, ConstantRange> range = new HashMap<String, ConstantRange>();
					range.put(embField.fieldName(), new VectorConstantRange(embField.query(), true, embField.query(), true));
					return new IndexSelectPlan(tablePlan, ii, range, tx);
				}
			}
